...
class AnalysisManager(threading.Thread):
    ...
    def build_options(self, update_with={}):
        """Generate analysis options. Add update_with to options being
        generated by default
        @param update_with: dict. These are added to the default options
        """
        default = {
        ...
        default.update(update_with)
        options = copy.deepcopy(self.task.options)
        if "options" in update_with:
            options.update(update_with["options"])
        default["options"] = emit_options(options)
    ...
    def on_status_init(self, db):
        """Is called by the scheduler thread when analysis status is init.
